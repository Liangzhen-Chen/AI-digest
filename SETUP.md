...
RSS Feeds (BBC, YouTube, TechCrunch, 人人都是产品经理 ...)
   ...
   feeds.py  (采集 + 过滤)
        ↓
 summarizer.py  (Gemini 2.0 Flash 免费 API 生成中文简报)
        ↓
   mailer.py  (Gmail SMTP 发送 HTML 邮件)
        ↓
 GitHub Actions  (每天伦敦时间 9:00 AM 自动触发)
```
...
接下来要添加 **4 个 Secret**，每个都是点「New repository secret」→ 填写 → 保存，重复 4 次：
...
添加完毕后，你应该能在 Secrets 页面看到 4 条记录（只显示名字，不显示值，这是正常的）。
...
配置成功后，GitHub Actions 会在 **每天伦敦时间早上 9:00** 自动运行发送简报。

> 注意：英国有夏令时（3月最后一个周日 → 10月最后一个周日），当前 cron 设置为 UTC 8:00，对应夏令时 BST 9:00。冬令时期间实际会在伦敦时间 8:00 发送。如果想精确到冬令时也是 9:00，可以在每年 10 月底手动把 `daily-digest.yml` 里的 cron 改为 `"0 9 * * *"`，3 月底再改回 `"0 8 * * *"`。
...
| 组件 | 费用 |
|---|---|
| Gemini 2.0 Flash API | 免费（15 RPM / 100万 token/天） |
